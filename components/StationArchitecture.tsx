...
const STATION_DATA_EN = {
    ...
    ess2: {
        ...
        bat: { 
            name: '#2', soc: '3%', soh: '99.5%', status: 0,
...
const getArchTheme = (isDark: boolean) => ({
    ...
    canvas: isDark
        ? 'bg-apple-bg-dark border-0 shadow-none'
        : 'bg-apple-bg-light border-0 shadow-none',
    /** 亮色下去除灰底点阵；暗色保留弱网格 */
    gridDot: isDark ? 'rgba(148,163,184,0.14)' : 'transparent',
    ...
    cardShell: isDark
        ? 'bg-zinc-900/95 border-0 shadow-none'
        : 'bg-white border-0 shadow-[0_4px_12px_rgba(0,0,0,0.06)]',
    cardHeader: isDark ? 'bg-brand-600 text-white' : 'bg-brand-100 text-brand-900',
    textMain: isDark ? 'text-white' : 'text-slate-900',
    textSub: isDark ? 'text-zinc-400' : 'text-slate-500',
    pillSolid: isDark ? 'bg-brand-600 text-white border-0' : 'bg-brand-600 text-white border-0',
    pcsOutline: isDark ? 'border-0 bg-black/40 text-brand-400' : 'border-0 bg-white text-brand-700',
});
...
const NodeCard = ({
    ...
    const variants: Record<CardVariant, string> = {
        ...
        orange: 'bg-orange-50 border-0',
        blue: 'bg-blue-50 border-0',
        emerald: 'bg-emerald-50 border-0',
        slate: 'bg-slate-100 border-0',
        purple: 'bg-purple-50 border-0',
    };
    ...
    return (
        <div 
            className={`${
                archShell ?? `${baseClass} dark:bg-apple-surface-dark dark:border-0 shadow-sm`
            } rounded-2xl border-0 p-3 absolute z-10 ${className}`}
            style={style}
        >
            <div className={`mb-2 flex items-center gap-2 border-b-0 pb-2`}>
                <div
...
const PccMeterCard = ({
    ...
    return (
        <div
            className={`${archShell} relative z-10 flex w-[100px] shrink-0 flex-col items-center justify-center gap-2 rounded-xl border-0 p-2 ${className}`}
            style={style}
        >
...
const PcsCard = ({
    ...
    return (
        <div className="absolute z-10 flex w-[6.75rem] flex-col items-center justify-center overflow-visible" style={style}>
            ...
            />
            <div
                className={`w-20 shrink-0 rounded-2xl border-0 shadow-lg relative z-10 flex flex-col items-center justify-center gap-1 py-1.5 ${box}`}
            >
                ...
                <div className="absolute left-[100%] top-0 ml-3 flex flex-col gap-1.5 w-max items-start justify-center h-full">
                     {data.status.mode && (
                        <div title="Mode">
                            <span
                                className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md border-0 ${
                                    isDark ? 'bg-zinc-800 text-zinc-200' : 'bg-slate-100 text-slate-600'
                                }`}
                            >
                                {data.status.mode}
                            </span>
...
/** 图解样式：SOC 数字与「/ SOC」叠在 SOC 条内；其下为「SOH」左、数值右 */
const BessSocSohStrip = ({ soc, soh, isDark }: { soc: string; soh: string; isDark: boolean }) => {
    const pct = Math.min(100, Math.max(0, parseFloat(soc.replace('%', '')) || 0));
    const lowSoc = pct <= 20;
    const accent = lowSoc ? '#fb923c' : '#809030';
    const labelMute = isDark ? 'text-zinc-500' : 'text-slate-400';
    const sohTone = isDark ? 'text-zinc-100' : 'text-slate-800';
    const trackBg = isDark ? 'bg-zinc-800/90' : 'bg-slate-100';
    /** 电池右侧正极小帽（比轨略深，略窄于主体高度） */
    const capBg = isDark ? 'bg-zinc-700' : 'bg-slate-300';
    /** 条内主读数：纯色轨与橄榄/橙色填充上都保持对比（不再用同色 accent 填字以免满条时糊） */
    const inBarMain = isDark
        ? 'text-zinc-50 drop-shadow-[0_1px_1px_rgba(0,0,0,0.5)]'
        : 'text-slate-900 drop-shadow-[0_1px_0_rgba(255,255,255,0.45)]';
    const inBarSub = isDark ? 'text-zinc-400' : 'text-slate-600';

    return (
        <div className="space-y-2">
            <div className="flex h-10 w-full items-center">
                <div
                    className={`relative h-full min-w-0 flex-1 overflow-hidden rounded-l-md rounded-r-none ${trackBg}`}
                >
                    <div
                        className="h-full min-w-0 rounded-l-md rounded-r-none transition-[width] duration-300 ease-out"
                        style={{ width: `${pct}%`, backgroundColor: accent }}
                    />
                    <div className="pointer-events-none absolute inset-0 z-10 flex items-center gap-1 px-2.5">
                        <span className={`font-mono text-2xl font-black tabular-nums tracking-tight ${inBarMain}`}>
                            {Math.round(pct)}%
                        </span>
                        <span className={`translate-y-px text-[11px] font-normal ${inBarSub}`}>/ SOC</span>
                    </div>
                </div>
                <span
                    aria-hidden
                    className={`pointer-events-none h-6 w-1.5 shrink-0 rounded-sm rounded-l-none rounded-r-[3px] ${capBg}`}
                />
            </div>
            <div className="flex items-baseline justify-between gap-2 leading-none">
                <span className={`text-xs font-bold uppercase tracking-wide ${labelMute}`}>SOH</span>
                <span className={`min-w-0 font-mono text-base font-semibold tabular-nums text-right ${sohTone}`}>{soh}</span>
            </div>
        </div>
    );
};
...
const ArchBessCard = ({
    ...
    const th = getArchTheme(isDark);
    const metricIconPlate =
        isDark ? 'bg-zinc-800/95 text-brand-400' : 'bg-brand-50 text-brand-700';

    return (
        <div className={`absolute z-10 flex w-[180px] flex-col items-stretch ${className}`} style={style}>
            <div className={`w-full overflow-hidden rounded-2xl border-0 ${th.cardShell}`}>
                <div className="p-2.5">
                    <div className="mb-2 flex items-center gap-2 border-b-0 pb-2">
                        <div
                        ...
                        <span
                            className={`min-w-0 text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}
                        >
                            {title}
                        </span>
                    </div>
                    ...
                    <BessSocSohStrip soc={soc} soh={soh} isDark={isDark} />
                    <div className="flex gap-2 rounded-xl p-0">
                        <div
                            className={`flex h-11 w-7 shrink-0 items-center justify-center rounded-md border-0 text-base font-black leading-none ${metricIconPlate}`}
                        >
                            V
                        </div>
                        <div className="min-w-0 flex-1 space-y-1 text-xs">
                            <div className={`flex items-center justify-between gap-1 ${th.textMain}`}>
                                ...
                                <span className="flex items-center gap-0.5 font-mono font-bold">
                                    {vmax}
                                    <ArrowUp className="shrink-0 text-red-500" size={13} strokeWidth={2.5} />
                </span>
                            </div>
                            <div className={`flex items-center justify-between gap-1 ${th.textMain}`}>
                                ...
                                <span className="flex items-center gap-0.5 font-mono font-bold">
                                    {vmin}
                                    <ArrowDown className="shrink-0 text-brand-500" size={13} strokeWidth={2.5} />
                                </span>
                            </div>
                        </div>
                    </div>
                    <div className="flex gap-2 rounded-xl p-0">
                        <div
                            className={`flex h-11 w-7 shrink-0 items-center justify-center rounded-md border-0 ${metricIconPlate}`}
                        >
                            <Thermometer size={20} strokeWidth={2.25} />
                        </div>
                        <div className="min-w-0 flex-1 space-y-1 text-xs">
                            <div className={`flex items-center justify-between gap-1 ${th.textMain}`}>
...
const DataRow = ({ label, value, subValue, highlight = false }: any) => {
    ...
    return (
        ...
        <div className="flex items-center gap-1">
                {isStatusLabel ? (
                    <span className="rounded-md border-0 bg-brand-50 px-2 py-0.5 text-[11px] font-bold text-brand-700 dark:bg-brand-900/30 dark:text-brand-300">
                {value}
            </span>
                ) : (
                    <span
...
const StationDiagram = ({
    ...
    if (view === 'overview') {
        ...
        const diagramSurfaceStyle: React.CSSProperties = {
            ...
            height: HEIGHT,
            ...(isDark
                ? { backgroundColor: 'var(--color-apple-bg-dark)' }
                : { backgroundColor: 'rgb(240, 240, 240)' }),
        };

        return (
            <div className={`relative overflow-hidden rounded-xl ${th.canvas}`} style={diagramSurfaceStyle}>
                {isDark && (
                <div
                ...
                >
                    <div className="flex items-center gap-4 rounded-2xl border-0 bg-brand-600 p-4 text-white shadow-xl">
                ...
                >
                    <div
                        className={`flex flex-col items-center gap-2 rounded-2xl border-0 p-5 shadow-lg ${
                            isDark
                ...
                <div style={{ position: 'absolute', top: Y_DEVICE_GROUPS, left: X_GRID, transform: 'translateX(-50%)' }} className="w-56">
                    <div className={`space-y-4 rounded-3xl border-0 p-4 ${th.cardShell}`}>
                        <div className={`flex items-center gap-2 border-b-0 pb-2`}>
                        ...
                        <div className={`rounded-xl border-0 p-3 ${isDark ? 'bg-zinc-950/50' : 'bg-slate-50/80'}`}>
                    ...
                    <div className={`space-y-4 rounded-3xl border-0 p-4 ${th.cardShell}`}>
                        ...
                        <div className="space-y-2">
                            {[1, 2, 3].map((i) => (
                                <div
                                    key={i}
                                    className={`overflow-hidden rounded-xl border-0 ${isDark ? 'bg-zinc-950/50' : 'bg-white'}`}
                                >
                                    <div className={`px-2.5 pt-2 pb-1 text-center text-[10px] font-black ${th.textMain}`}>
                                        BESS #{i}
                                    </div>
    ...
    /** 并网馈线三列卡片与 FloatingData 的留白；SVG 垂线末端须与该值一致以保持与卡片顶相接 */
    const MAIN_FEEDER_CARD_TOP_GAP = 24;
    const Y_MAIN_FEEDERS_LINE_END = Y_MAIN_ROW + MAIN_FEEDER_CARD_TOP_GAP;
    const Y_DC_START = Y_MAIN_ROW + 80; 
    /** 随 AC 段落下移，直流桩出线起点同步下移*/
    const Y_EVSE_DC_START = Y_MAIN_ROW + 100 + MAIN_FEEDER_CARD_TOP_GAP;
    ...
    return (
        <div className={`relative overflow-visible rounded-xl ${th.canvas}`} style={diagramSurfaceStyle}>
        {isDark && (
            <div
            ...
            <svg className="pointer-events-none absolute inset-0 z-0 h-full w-full overflow-visible rounded-xl">
                ...
                <path
                    d={`M${X_DG + OFFSET_STD} ${Y_BUSBAR} V ${Y_MAIN_FEEDERS_LINE_END}`}
                    stroke={acTap}
                    strokeWidth={2}
                    strokeDasharray="4 4"
                    fill="none"
                />
                <path
                    d={`M${X_ACPV + OFFSET_STD} ${Y_BUSBAR} V ${Y_MAIN_FEEDERS_LINE_END}`}
                    stroke={acTap}
                    strokeWidth={2}
                ...
                />
                <path
                    d={`M${X_EVSE + OFFSET_STD} ${Y_BUSBAR} V ${Y_MAIN_FEEDERS_LINE_END}`}
                    stroke={acTap}
                    strokeWidth={2}
                    fill="none"
                />
                ...
                <SvgLineSwitch
                    x={X_DG + OFFSET_STD}
                    y={Y_BUSBAR + (Y_MAIN_FEEDERS_LINE_END - Y_BUSBAR) * 0.42}
                    isClosed={false}
                    isDark={isDark}
                />
                <SvgLineSwitch
            ...
            <div className="absolute left-[50%] top-0 z-30 flex -translate-x-1/2 flex-col items-center gap-1.5">
                <div
                    className={`flex items-center gap-2 rounded-xl border-0 px-3 py-2 shadow-lg ${th.pillSolid}`}
                >
            ...
            <div style={{ position: 'absolute', top: Y_MAIN_ROW, left: X_DG, transform: 'translateX(-50%)', zIndex: 10 }}>
                ...
                <NodeCard
                    ...
                    archIsDark={isDark}
                    className="w-[168px]"
                    style={{ position: 'relative', marginTop: MAIN_FEEDER_CARD_TOP_GAP }}
                    variant="slate"
                >
                    <div className="mb-2 flex gap-2">
                        <span
                            className={`rounded-md border-0 px-2 py-0.5 text-xs font-bold ${
                                isDark ? 'bg-orange-500 text-black' : 'bg-slate-100 text-slate-600'
                            }`}
                        >
                            Standby
                        </span>
                ...
                >
                    <div className="mt-1 grid grid-cols-2 gap-1.5">
                        {stationData.dcPv.panels.map((p: any) => (
                            <div
                                key={p.id}
                                className={`rounded-lg border-0 p-1.5 ${
                                    isDark ? 'bg-zinc-950/60' : 'bg-slate-50'
                                }`}
                            >
                                <div className={`text-xs font-bold ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{p.id}</div>
                                <div className={`font-mono text-sm leading-tight ${isDark ? 'text-zinc-200' : 'text-slate-700'}`}>{p.p}</div>
                                <div className={`text-xs ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{p.v}/{p.i}</div>
                            </div>
                        ))}
                    </div>
                </NodeCard>
...
const StationArchitecture: React.FC<StationArchitectureProps> = ({ lang, theme, selectedStation }) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-apple-bg-light dark:bg-apple-bg-dark text-slate-800 dark:text-slate-200 font-sans selection:bg-blue-100 dark:selection:bg-blue-500/30 flex flex-col transition-all duration-300 ${isFullScreen ? 'fixed inset-0 z-[100] w-screen h-screen' : 'w-full h-[calc(100vh-72px)]'}`}>
